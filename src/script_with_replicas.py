...
class ConsistentHashRing:
	...
	def addReplicas(self,machineIndex):
		
		self.replicas[machineIndex] = []
		for replicaIndex in xrange(self.numReplicas):
		
			newMachine = {}
			newMachine['index'] = machineIndex
			newMachine['key'] = str(machineIndex) + "::" + str(replicaIndex)
			newMachine['hash'] = self._getHash(newMachine['key'])

			self.replicas[machineIndex].append(newMachine)
	...
	def addMachine(self):
		...
		self.machineTuples.append(newMachine)
		self.addReplicas(self.machineIndexCounter - 1)
		self.sortMachines()
